...
local function RefreshPAWorkerEventRegistration()
    ...
    if PAW and PAPM.PAWorker and PAPM.PAWorker.hasActiveProfile() then
		...
		if autoDeconstruct or autoRefine then
		    RegisterForEvent(PAW.AddonName, EVENT_CRAFTING_STATION_INTERACT, function( _, craftSkill, sameStation, craftMode) PAW.StartCraftingInterraction(craftSkill, sameStation, craftMode, autoDeconstruct, autoRefine) end)
