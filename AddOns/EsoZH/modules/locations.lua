function EsoZh_doubleNameLocation(EsoZH)
	...
	function GetMapLocationTooltipHeader(...)
		...
		newLocName = EsoZH.Names.Locations[zo_strlower(headerText)]
	...
	function GetPOIInfo(...)
		...
		newLocName = EsoZH.Names.Locations[zo_strlower(poiName)]
	...
	function GetMapMouseoverInfo(...)
		...
		newLocName = EsoZH.Names.Locations[zo_strlower(locationName)]
	...
	function GetMapName(...)
		...
		newLocName = EsoZH.Names.Locations[zo_strlower(zoneName)]
	...
	function GetFastTravelNodeInfo(...)   --大地图快速传送点
		...
		newLocName = EsoZH.Names.Locations[zo_strlower(name)]
	...
	function GetMapInfoByIndex(...)    --大地图地点名（UI右侧）
		...
		newLocName = EsoZH.Names.Locations[zo_strlower(mapName)]
	...
	function GetFriendCharacterInfo(...)
		...
		newLocName = EsoZH.Names.Locations[zo_strlower(zone)]
	...
	ZO_PostHook(FRIENDS_LIST_MANAGER, "OnFriendCharacterZoneChanged", function(tooltip, displayName, characterName, zoneName)
		...
		if data then
			...
			local newLocName = EsoZH.Names.Locations[zo_strlower(oldLocation)]
		...
		handlers[EVENT_ZONE_CHANGED] = function(zoneName, subzoneName)
			if(subzoneName ~= "") then
				if (EsoZH.Settings.LocationDoubleName ~= "zh") then
					newLocName = EsoZH.Names.Locations[zo_strlower(subzoneName)]
...
function LFGDoubleNames(EsoZH)
	...
	for i = 1, #locs2 do	   
	   newLocationName = EsoZH.Names.Locations[zo_strlower(locs2[i]["rawName"])]
	...
	for i = 1, #locs3 do	   
	   newLocationName = EsoZH.Names.Locations[zo_strlower(locs3[i]["rawName"])]
	   ...
	   if newLocationName ~= nil then
			if (EsoZH.Settings.LocationDoubleName == "zh") then
				...
				if string.find(locs3[i]["nameGamepad"], "^老兵") then
					locs3[i]["nameGamepad"] = "老兵 " .. locs3[i]["rawName"]
				end
			else
				if string.find(locs3[i]["nameKeyboard"], "target_veteranRank_icon") then
				...
				if string.find(locs3[i]["nameGamepad"], "^老兵") then
					locs3[i]["nameGamepad"] = "老兵 " .. locs3[i]["rawName"].. " (" .. newLocationName .. ")"
				end
			end
		end
	end
