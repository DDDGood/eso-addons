...
local function OnCombatEvent( _, result, _, _, _, _, sourceName, sourceType, targetName, targetType, hitValue, powerType, damageType, log, sourceUnitId, targetUnitId, abilityId, overflow )
	...
	if abilityId == TOTEM_GARGYL and BSCHTKA.SV_ACC.GARGYL_TOTEM then -- Gargyl Totem
		local cid = -1
		if BSCHTKA.SV_ACC.USE_COLOR then
	...
	if abilityId == YANDIR_HEALING and result == ACTION_RESULT_BEGIN and BSCHTKA.SV_ACC.HEALING_YANDIR then
		if BSCHTKA.SV_ACC.USE_COLOR then
	...
	if abilityId == YANDIR_JUMP and result == ACTION_RESULT_BEGIN and BSCHTKA.SV_ACC.JUMP_YANDIR then
		local cid = -1
		if BSCHTKA.SV_ACC.USE_COLOR then
	...
	--and zo_strformat("<<1>>", GetUnitName('player')) == zo_strformat("<<1>>", targetName)
	if abilityId == SEA_ADDER_BILE_SPRAY and BSCHTKA.SV_ACC.SEA_ADDER_BILE_SPRAY then
		local cid = -1
		if BSCHTKA.SV_ACC.USE_COLOR then
